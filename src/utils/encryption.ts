...
try {
// В React Native нет глобальных функций btoa/atob, используем Buffer
...
export const STORAGE_KEYS = {
  ...
  ADMIN_PASSWORD: 'encrypted_admin_password', // Новый ключ для пароля администратора
};
...
export const initializeDefaultValues = async (): Promise<void> => {
  try {
    ...
    if (!ipsecKey) {
      await saveEncryptedValue(STORAGE_KEYS.IPSEC_KEY, 'vpnpn');
    }
